          ...
          {categories &&
            categories.map((category) => (
              <motion.div
                ...
                id={category.id}
                className={`group ${
                  filter === category.urlPname ? "bg-cartNumBg" : "bg-white"
                ...
                onClick={() => setFilter(category.urlPname)}
              >
                <div
                  className={`w-10 h-10 rounded-full shadow-lg ${
                    filter === category.urlPname ? "bg-white" : "bg-cartNumBg"
                  } group-hover:bg-white flex items-center justify-center`}
                >
                  <IoFastFood
                    className={`${
                      filter === category.urlPname
                        ? "text-textColor"
                        : "text-white"
